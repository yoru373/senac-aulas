...
class DadosUserPage extends StatefulWidget {
  const DadosUserPage({super.key}); // Construtor com chave opcional (boa prática para widgets)
...
class _DadosUserPageState extends State<DadosUserPage> {
 ...
 void staloadSharedPreferences() async {
  String? loginStorage = await SharedPreferencesServices.getStringlocalStorage("login");
  String? senhaStorage = await SharedPreferencesServices.getStringlocalStorage("senha");
setState(() {
login =  loginStorage;
senha =  senhaStorage;
